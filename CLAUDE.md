...
| Région | Fonctions Vercel et base Upstash en `fra1` (Francfort), fixées dans `vercel.json`. Pennylane est en France et un appel d'outil peut déclencher jusqu'à 10 appels Pennylane : mieux vaut un seul aller-retour lointain, entre le client et le serveur, que dix entre le serveur et Pennylane. Les données comptables sont traitées en Europe, base et fonctions dans la même région |
| Outils `analyze_*` | Supprimés : ils calculent des totaux sur la première page. Remplacés par `pennylane_get_trial_balance` |
| Interprétation | Les outils de niveau 1 sont des relais fins. Toute dérivation vit dans un point unique, nommé et testé (`lib/derivations.js`) : 3 bugs sur 4 sont nés dans les deux seuls outils qui dérivaient une valeur. Le solde d'une ligne de balance se calcule en centimes entiers à partir des chaînes de l'API, jamais en flottants, sans aucun total sur l'ensemble des lignes |
